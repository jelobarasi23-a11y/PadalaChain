...
import RemittanceABI from "../lib/abi/PadalaRemittance.json";
  ...
  async function fetchLogs() {
    ...
    try {
      ...
      const contract = new ethers.Contract(
        process.env.NEXT_PUBLIC_REMITTANCE_ADDRESS,
        RemittanceABI,
        provider
      );
      const raw = await contract.getMyLogs(familyAddress);
      setLogs(raw.map(l => ({
        sender:   l.sender,
        amount:   ethers.formatUnits(l.amount, 18),
        category: Number(l.category),
        date:     new Date(Number(l.timestamp)*1000).toLocaleDateString("en-PH", {
      ...
      if (e.code === "UNSUPPORTED_OPERATION") {
        // ENS not supported on Morph — safe to ignore
        setLoaded(true);
      } else {
        console.error(e);
      }
    ...
    <div>
      ...
      {loaded && logs.length === 0 && (
        <div style={{
          ...
          <div style={{ fontSize:"12px", color:"#374151", marginTop:"4px" }}>Send one from the left panel!</div>
        </div>
      )}
              ...
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{
                  ...
                  <span style={{
                    ...
                    background: c.bg, padding:"2px 7px",
